...
微博用户身份分类器 — 评论区特化版
===================================
基于 weibo_user_identity.py v3，适配评论区CSV格式 + 五人均分。

输入：predicted_stance/weibo_comments_all_predicted.csv
      (列: user_id, user_name, comment_text, like_count, reply_count,
            comment_time, predicted_stance, prediction_source ...)

五类群体：张碧晨粉丝, 张方水军, 汪苏泷粉丝, 汪方水军, 路人

五人分片用法（每人跑自己的片）：
  python weibo_user_identity_comments.py --slice 1    # 第1份
  python weibo_user_identity_comments.py --slice 2    # 第2份
  python weibo_user_identity_comments.py --slice 3    # 第3份
  python weibo_user_identity_comments.py --slice 4    # 第4份
  python weibo_user_identity_comments.py --slice 5    # 第5份

每个片约 12,110 用户，分片由 user_id 哈希确定，互不重叠。

设好 COOKIE 后默认 Phase 1 → 自动接 Phase 2。
不带 Cookie 则只跑 Phase 1（纯文本分析，无需登录）。
...
def iter_progress(items, desc, unit='条', total=None):
    """带 ETA 的进度条；无 tqdm 时退化为普通迭代"""
    n = total if total is not None else (len(items) if hasattr(items, '__len__') else None)
    if tqdm is None:
        return items
    return tqdm(
        items, desc=desc, total=n, unit=unit,
...
def find_input_csv():
    """自动查找评论区CSV"""
    candidates = [
        Path(__file__).parent.parent / "predicted_stance" / "weibo_comments_all_predicted.csv",
        Path.cwd() / "predicted_stance" / "weibo_comments_all_predicted.csv",
        Path.cwd() / "weibo_comments_all_predicted.csv",
    ]
    for p in candidates:
        if p.exists():
            return p
...
def safe_str(s, max_len=50):
    ...
    for ch in s:
        ...
        if (0x4e00 <= cp <= 0x9fff or
            0x20 <= cp <= 0x7e or
            cp in (0x3001, 0x3002, 0xff0c, 0xff0e, 0xff01, 0xff1f,
                   0xff1a, 0xff1b, 0x2018, 0x2019, 0x201c, 0x201d,
...
def belongs_to_slice(user_id, slice_num, total=5):
    """确定性分片：同一 user_id 总是属于同一片"""
    return hash(str(user_id).strip()) % total == slice_num - 1
...
def map_comment_row(row):
    """
    将评论区CSV行映射为 weibo_user_identity.py 期望的内部格式。
    评论区列名: user_id, user_name, comment_text, comment_time,
                like_count, reply_count, predicted_stance, prediction_source
    """
    return {
        'userid': row.get('user_id', '').strip(),
        'author_name': row.get('user_name', '').strip(),
        'author_type': '',  # 评论区无此列，统一为空
        'text': row.get('comment_text', '').strip(),
        'predicted_stance': row.get('predicted_stance', '').strip(),
        'publish_time': row.get('comment_time', '').strip(),
        'repost_count': 0,                      # 评论区无转发
        'comment_count': row.get('reply_count', '0').strip() or '0',
        'like_count': row.get('like_count', '0').strip() or '0',
        # 保留原始数据以备回写
        '_raw': row,
    }
...
def aggregate_users(rows):
    ...
    for row in rows:
        ...
        u = users[uid]
        u['userid'] = uid
        u['author_name'] = row['author_name']
        u['author_type_orig'] = row['author_type']
        ...
        try:
            u['repost_counts'].append(int(row['repost_count'] or 0))
            u['comment_counts'].append(int(row['comment_count'] or 0))
            u['like_counts'].append(int(row['like_count'] or 0))
        except ValueError:
            pass
...
def crawl_profile_info(uid):
    ...
    uid = str(uid).removesuffix('.0')  # 去掉 CSV 中 float 格式的 .0 后缀
...
def run_phase1(input_csv, slice_num, slice_total, do_slice=True):
    print("=" * 60)
    if do_slice:
        print(f"Phase 1: 文本+行为分类 — 第 {slice_num}/{slice_total} 片")
    else:
        print(f"Phase 1: 文本+行为分类 — {input_csv.name}")
    print("=" * 60)

    # 1. 加载 + (可选)分片过滤
    print(f"\n[1/5] 加载: {input_csv}")
    all_rows = load_csv(str(input_csv))
    print(f"  总行数: {len(all_rows)}")

    if do_slice:
        print(f"\n[1.5/5] 分片过滤: 只保留 user_id hash % {slice_total} == {slice_num - 1}")
        mapped_rows = []
        my_users = set()
        skipped = 0
        for raw in iter_progress(all_rows, desc='[1.5/5] 分片过滤', unit='行'):
            uid = raw.get('user_id', '').strip()
            if not uid:
                skipped += 1
                continue
            if belongs_to_slice(uid, slice_num, slice_total):
                mapped_rows.append(map_comment_row(raw))
                my_users.add(uid)
        print(f"  本片: {len(mapped_rows)} 行, {len(my_users)} 唯一用户")
        print(f"  跳过(无user_id): {skipped}")
    else:
        print(f"\n[1.5/5] 跳过片过滤(预拆分文件)")
        mapped_rows = []
        skipped = 0
        for raw in iter_progress(all_rows, desc='[1.5/5] 读取映射', unit='行'):
            uid = raw.get('user_id', '').strip()
            if not uid:
                skipped += 1
                continue
            mapped_rows.append(map_comment_row(raw))
        print(f"  有效: {len(mapped_rows)} 行")
        print(f"  跳过(无user_id): {skipped}")

    # 2. 聚合
    print("\n[2/5] 按 user_id 聚合...")
    users = aggregate_users(mapped_rows)
    ...
    print("\n[4/5] 五类身份分类...")
    results = {}
    for uid, u in iter_progress(list(users.items()), desc='[4/5] 身份分类', unit='人'):
    ...
    # 输出1: 每用户详情
    # 从文件名提取片号后缀（如 weibo_comments_slice4of5.csv → _slice4of5）
    import re as _re
    _m = _re.search(r'slice(\d+of\d+)', str(input_csv.name))
    if _m:
        sfx = f'_slice{_m.group(1)}'
    elif do_slice:
        sfx = f'_slice{slice_num}of{slice_total}'
    else:
        sfx = ''
    detail_file = OUTPUT_DIR / f'user_identity_v3{sfx}.csv'
    with open(detail_file, 'w', encoding='utf-8-sig', newline='') as f:
        ...
        writer.writeheader()
        for uid, r in iter_progress(list(results.items()), desc='[5/5] 写用户详情', unit='人'):
            u = users[uid]
    ...
    # 输出2: 原始格式 + author_type 细化（含所有列）
    enriched_file = OUTPUT_DIR / f'weibo_comments_enriched{sfx}.csv'
    identity_map = {uid: r['author_type'] for uid, r in results.items()}

    # 用原始行回写，新增 author_type 列
    enriched_rows = []
    for row in iter_progress(mapped_rows, desc='[5/5] 写 enriched', unit='行'):
        new_row = dict(row['_raw'])
        uid = row['userid']
        # 添加/覆盖 author_type
        new_row['author_type'] = identity_map.get(uid, '')
        enriched_rows.append(new_row)

    if enriched_rows:
        write_csv(enriched_file, enriched_rows, list(enriched_rows[0].keys()))
    ...
    if low_conf:
        check_file = OUTPUT_DIR / f'low_confidence_check{sfx}.csv'
        ...
        print(f"  提示: 设置 COOKIE 后可跑 Phase 2 修正这 {len(low_conf)} 人")
    else:
        check_file = OUTPUT_DIR / f'low_confidence_check{sfx}.csv'
        check_file.write_text('userid,author_name,author_type,dominant_stance,n_posts,confidence,evidence\n', encoding='utf-8-sig')

    if water_users:
        water_file = OUTPUT_DIR / f'water_army_verify{sfx}.csv'
        with open(water_file, 'w', encoding='utf-8-sig', newline='') as f:
            ...
            for uid, r in water_users:
                ...
                writer.writerow([uid, u['author_name'], r['author_type'],
                               get_dominant_stance(u['stances']), len(u['texts']),
                               r['confidence'], '; '.join(r['evidence'])])
        print(f"  水军验证名单 → {water_file} ({len(water_users)} 人)")

    print(f"\nPhase 1 (第{slice_num}/{slice_total}片) 完成！")
...
def _autosave_results(results, refined_detail, sfx):
    """每批次自动保存 Phase 2 结果，防止中途崩溃丢数据"""
    if not results:
        return
    # 1. 保存 Phase 2 爬取详情
    tmp_file = OUTPUT_DIR / f'user_identity_homepage_refined{sfx}.csv'
    write_csv(tmp_file, results, list(results[0].keys()))
    # 2. 更新 v3 用户详情
    v3_file = OUTPUT_DIR / f'user_identity_v3{sfx}.csv'
    if v3_file.exists():
...
def run_phase2(input_csv, slice_num, slice_total, include_water=False):
    global COOKIE
    ...
    # 从文件名提取片号后缀
    _m = re.search(r'slice(\d+of\d+)', str(input_csv.name))
    if _m:
        sfx = f'_slice{_m.group(1)}'
    else:
        sfx = f'_slice{slice_num}of{slice_total}'
    ...
    print("=" * 60)
    print(f"Phase 2: 主页爬取 + 精细化分类 — 第 {slice_num}/{slice_total} 片")
    if include_water:
        print("模式: 低置信度 + 水军验证")
    print("=" * 60)
    ...
    est_sec = len(crawl_users) * (avg_delay * 3 + 2) + (len(crawl_users) // BATCH_SIZE) * BATCH_REST
    print(f"预计 Phase 2 耗时约 {est_sec / 3600:.1f} 小时 ({est_sec / 60:.0f} 分钟)\n")
    ...
    pbar = iter_progress(crawl_users, desc='Phase2 爬主页', unit='人')
    if tqdm and hasattr(pbar, 'set_postfix_str'):
        pbar.bar_format = '{desc}: {percentage:3.0f}%|{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}] {postfix}'
    ...
    for i, user in enumerate(pbar):
        ...
        if new_type != old_type:
            refined_count += 1
            log_progress(f"  修正 {uname}: {old_type} -> {new_type}")

        if not profile:
            errors.append(uid)
            log_progress(f"  [FAIL] {uname} 资料获取失败")
        ...
        if tqdm and hasattr(pbar, 'set_postfix_str'):
            pbar.set_postfix_str(f'修正{refined_count} 失败{len(errors)}', refresh=True)

        if (i + 1) % BATCH_SIZE == 0:
            log_progress(f"--- 批次完成 ({i+1}/{len(crawl_users)})，休息 {BATCH_REST}s，自动保存中... ---")
            # 自动保存：防止中途崩溃丢数据
            _autosave_results(results, refined_detail, sfx)
    ...
    result_file = OUTPUT_DIR / f'user_identity_homepage_refined{sfx}.csv'
    ...
    if enriched_file.exists():
        ...
        for row in rows:
            ...
            if uid in refined_detail:
                row['author_type'] = refined_detail[uid]['author_type']
        if rows:
            write_csv(enriched_file, rows, list(rows[0].keys()))
...
def merge_all_slices():
    ...
    fieldnames = None
    for i in range(1, 6):
        f = OUTPUT_DIR / f'weibo_comments_enriched_slice{i}of5.csv'
        if not f.exists():
            print(f"  [WARN] 缺失: {f}")
            continue
        rows = load_csv(str(f))
        print(f"  slice {i}: {len(rows)} 行")
    ...
    merged_file = OUTPUT_DIR / 'weibo_comments_enriched_ALL.csv'
    write_csv(merged_file, all_rows, fieldnames)
    print(f"\n  合并完成: {len(all_rows)} 行 → {merged_file}")

    # 合并 v3 用户详情
    v3_all = []
    v3_fields = None
    for i in range(1, 6):
        f = OUTPUT_DIR / f'user_identity_v3_slice{i}of5.csv'
        if not f.exists():
            continue
        rows = load_csv(str(f))
        if v3_fields is None:
            v3_fields = list(rows[0].keys())
        v3_all.extend(rows)

    if v3_all:
        v3_file = OUTPUT_DIR / 'user_identity_v3_ALL.csv'
        write_csv(v3_file, v3_all, v3_fields)
        print(f"  v3 合并完成: {len(v3_all)} 人 → {v3_file}")

        type_counts = Counter(r['author_type'] for r in v3_all)
        print(f"\n  === 全量五类分布 ===")
        for t, c in type_counts.most_common():
            print(f"    {t}: {c} ({c/len(v3_all)*100:.1f}%)")
...
def main():
    global COOKIE, SLICE_NUM, SLICE_TOTAL
    ...
    # --slice N（可选，仅当输入是完整大文件时需要）
    slice_num = None
    for a in args:
        if a.startswith('--slice='):
            slice_num = int(a.split('=')[1])
        elif a == '--slice':
            idx = args.index('--slice')
            if idx + 1 < len(args):
                slice_num = int(args[idx + 1])

    # 自动检测：文件名含 sliceXof5 → 预拆分文件，无需再切
    is_pre_split = '_slice' in str(csv_path.name) and 'of5' in str(csv_path.name)
    ...
    if slice_num is not None:
        ...
        slice_num = 1
        slice_total = 5
        print(f"检测到预拆分文件: {csv_path.name}，跳过分片过滤")
    else:
        do_slice = False
        slice_num = 1
        slice_total = 5
        print("提示: 未指定 --slice，处理整个输入文件")
    ...
    if phase2_only:
        run_phase2(csv_path, slice_num, slice_total, include_water=include_water)
    elif phase1_only:
        run_phase1(csv_path, slice_num, slice_total, do_slice=do_slice)
    else:
        run_phase1(csv_path, slice_num, slice_total, do_slice=do_slice)
